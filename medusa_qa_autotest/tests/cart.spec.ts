...
test('TC-CART-05: 장바구니 항목 삭제 후 소계가 정확히 재계산된다', async ({ page }) => {
  // 1) 서로 다른 €10 상품 2종을 담는다 (테스트가 스스로 전제 생성 — 독립성)
  await addProductToCart(page, 't-shirt');    // €10
  await addProductToCart(page, 'sweatshirt'); // €10

  // 2) 장바구니로 이동
  await page.goto(`${BASE}/cart`);

  // 3) 삭제 전 상태 고정: 라인 2개, Subtotal data-value=20
  const rows = page.locator('[data-testid="product-row"]');
  await expect(rows).toHaveCount(2);
  await expect(
    page.locator('[data-testid="cart-subtotal"]')
  ).toHaveAttribute('data-value', '20');

  // 4) 첫 번째 라인을 삭제한다.
  //    삭제 버튼은 testid가 없으므로 product-row 안의 유일한 <button>으로 잡는다.
  //    (썸네일은 <a>, 수량은 <select>라 구조상 버튼은 삭제 버튼뿐 — 교훈 #14)
  await rows.first().locator('button').click();

  // 5) ★ 시간이 아니라 "변화"를 기다린다 (교훈 #10, 상태 델타):
  //    Subtotal이 20 → 10으로 재계산될 때까지 자동 재시도 대기.
  await expect(
    page.locator('[data-testid="cart-subtotal"]')
  ).toHaveAttribute('data-value', '10');

  // 6) 검증: 라인은 1개만 남는다 (2 → 1, 경계값)
  await expect(rows).toHaveCount(1);

  // 7) 검증: 남은 라인의 합계 = Subtotal (재계산 정합성)
  const subtotalValue = await page
    .locator('[data-testid="cart-subtotal"]')
    .getAttribute('data-value');
  const remainingLineText = await page
    .locator('[data-testid="product-price"]')
    .first()
    .textContent();
  const remainingLine = parseFloat(remainingLineText!.replace(/[^0-9.]/g, ''));
  expect(parseFloat(subtotalValue!)).toBeCloseTo(remainingLine, 2);
});